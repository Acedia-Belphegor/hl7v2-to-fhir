...
medication.code = create_codeable_concept('107750602', 'ソリタ－Ｔ３号輸液５００ｍＬ', 'urn:oid:1.2.392.100495.20.2.74')
ratio = FHIR::Ratio.new
ratio.numerator = create_quantity(500, 'ml')
ratio.denominator = create_quantity(1, '本')
...
medication = FHIR::Medication.new
medication.id = SecureRandom.uuid
medication.code = create_codeable_concept('108010001', 'アドナ注（静脈用）５０ｍｇ', 'urn:oid:1.2.392.100495.20.2.74')
ratio = FHIR::Ratio.new
ratio.numerator = create_quantity(10, 'ml')
ratio.denominator = create_quantity(1, 'アンプル')
medication.amount = ratio
batch = FHIR::Medication::Batch.new
