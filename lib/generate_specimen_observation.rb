    ...
    def perform()
        ...
        get_segment_groups.each do |segments|
            ...
            if spm_segment.present?
                ...
                # SPM-17.検体採取日時
                if spm_segment[:specimen_collection_datetime].present?
                    collection = FHIR::Specimen::Collection.new
                    collection.collectedDateTime = spm_segment[:specimen_collection_datetime].first[:range_start_datetime][:time]
            ...
            if obr_segment.present?
                ...
                # OBR-7.検査/採取日時
                if obr_segment[:observation_datetime_].present? && specimen.collection.blank?
                    collection = FHIR::Specimen::Collection.new
                    collection.collectedDateTime = obr_segment[:observation_datetime_].first[:time]
                    specimen.collection = collection
                end
            end
            ...
            segments.select{|segment|segment[:segment_id] == 'OBX'}.each do |obx_segment|
                ...
                # OBX-14.検査日時
                if obx_segment[:datetime_of_the_observation].present?
                    observation.effectiveDateTime = DateTime.parse(obx_segment[:datetime_of_the_observation].first[:time])
                end
