...
class GenerateMedicationRequestPrescription < GenerateAbstract
  def perform()
    ...
    get_segment_groups.each do |segments|
      ...
      if rxe_segment[:pharmacy_treatment_suppliers_special_dispensing_instructions].present?
        medication_request.category.concat rxe_segment[:pharmacy_treatment_suppliers_special_dispensing_instructions].map{|element|generate_codeable_concept(element)}
      end
