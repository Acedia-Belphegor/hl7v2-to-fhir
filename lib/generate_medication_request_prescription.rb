...
class GenerateMedicationRequestPrescription < GenerateAbstract
  def perform()
    ...
    get_segment_groups.each do |segments|
      ...
      # ORC-2.依頼者オーダ番号
      medication_request.identifier << generate_identifier(orc_segment[:placer_order_number].first[:entity_identifier], "urn:oid:1.2.392.100495.20.3.11.1.#{get_medical_institution_code}")
      # ORC-4.依頼者グループ番号
      ...
      if rxe_segment[:give_amount_minimum].to_f.positive?
        ...
        # ⼒価区分コード
        dose.type = build_codeable_concept('1', '製剤量', 'urn:oid:1.2.392.100495.20.2.22')
      end
      ...
      if tq1_segment[:service_duration].present?
        ...
        timing_repeat.boundsDuration = duration
      ...
      if jami_usage.present?
        ...
        # 頓用
        dosage.asNeededBoolean = true if JamiUsages::as_needed?(jami_usage.code)
      end
